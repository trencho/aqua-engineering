 ...
 *
 * Composition with the address block is covered in ContactSection.spec.ts.
 */
...
describe('with no access key configured', () => {
  ...
  it('hides the form rather than showing one that cannot submit', async () => {
    const { wrapper } = await mountAt(ContactForm)
    expect(wrapper.find('form').exists()).toBe(false)
  })

  it('leaves behind no orphan controls', async () => {
    const { wrapper } = await mountAt(ContactForm)
    expect(wrapper.findAll('input, textarea, button, label')).toEqual([])
  })
})
...
describe('submission', () => {
  ...
  it('tells the endpoint which language the enquiry came from', async () => {
    const fetchSpy = vi
      .spyOn(globalThis, 'fetch')
      .mockResolvedValue(new Response('{}', { status: 200 }))
    const { wrapper } = await mountAt(ContactForm, 'mk')
