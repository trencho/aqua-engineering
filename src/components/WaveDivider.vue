...
/**
 * The wave-brush divider that opened who-we-are, licences and contact on the
 * original site. Nine overlapping paths on a 283.5x27.8 viewBox, stretched to
 * the full section width by preserveAspectRatio="none" — the aspect ratio is
 * meant to break, which is what gives the brushed edge its shape at any width.
 *
 * Paths lifted verbatim from the archived markup. Purely decorative, so it is
 * hidden from assistive technology and carries no title.
 */
defineProps<{
  /** Painted fill. The divider sits on top of the section it introduces, so
      this is the colour of whatever is ABOVE it, not the section itself. */
  fill: string
}>()
</script>
...
.wave svg {
  display: block;
  width: calc(100% + 1.3px); /* hides the sub-pixel seam at some zoom levels */
  height: 45px;
}
